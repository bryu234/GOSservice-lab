...
PROJECT_VOLUMES = gosservice_lab_arm_adm_rsyslog_config gosservice_lab_arm_user_rsyslog_config gosservice_lab_router_rsyslog_config gosservice_lab_dns_rsyslog_config gosservice_lab_web_rsyslog_config gosservice_lab_db_rsyslog_config gosservice_lab_dns_bind_config gosservice_lab_mail_postfix_config gosservice_lab_mail_dovecot_config gosservice_lab_mail_rsyslog_config gosservice_lab_espocrm_data gosservice_lab_espocrm_custom gosservice_lab_espocrm_client_custom gosservice_lab_website_nginx gosservice_lab_espocrm_db gosservice_lab_mail_home gosservice_lab_suricata_logs gosservice_lab_router_firewall_state gosservice_lab_wazuh_certs gosservice_lab_wazuh_api_configuration gosservice_lab_wazuh_etc gosservice_lab_wazuh_logs gosservice_lab_wazuh_queue gosservice_lab_wazuh_var_multigroups gosservice_lab_wazuh_integrations gosservice_lab_wazuh_active_response gosservice_lab_wazuh_agentless gosservice_lab_wazuh_wodles gosservice_lab_filebeat_etc gosservice_lab_filebeat_var gosservice_lab_wazuh_indexer_data gosservice_lab_wazuh_dashboard_config gosservice_lab_wazuh_dashboard_custom

.PHONY: help env check-ports check-network check-wazuh-prereqs wazuh-certs config build up up-vm up-local fill-db fill-db-local down ps logs restart restart-router router-firewall shell-adm shell-user shell-evil shell-router shell-web shell-mail shell-siem-manager shell-siem-indexer shell-siem-dashboard ssh-adm ssh-user ssh-evil ssh-router ssh-web suricata-alerts rdp-info clean-volumes clean-all
...
help:
	...
	@echo "  make ssh-web      SSH from gos_arm_adm to the training website"
	@echo "  make shell-router Open a shell inside gos_router"
	@echo "  make shell-web    Open a shell inside gos_web"
...
check-ports: env
	...
	for item in "ADM_RDP_HOST_PORT:adm RDP" "ADM_SSH_HOST_PORT:adm SSH" "USER_RDP_HOST_PORT:user RDP" "USER_SSH_HOST_PORT:user SSH" "EVIL_RDP_HOST_PORT:evil RDP" "EVIL_SSH_HOST_PORT:evil SSH" "WEBSITE_HTTP_HOST_PORT:Training website HTTP" "WAZUH_DASHBOARD_HOST_PORT:Wazuh Dashboard HTTPS"; do \
...
# Открывает shell внутри учебного web-контейнера.
shell-web: env
	@$(COMPOSE_DEFAULT) exec gos_web bash
...
# SSH учебного сайта не публикуется на host: подключаемся через adm-машину.
ssh-web: env
	@set -a; . ./$(ENV_FILE); set +a; \
	$(COMPOSE_DEFAULT) exec gos_arm_adm \
		ssh -o StrictHostKeyChecking=accept-new "$$LOCALADMIN_USER@site.$$GOS_DOMAIN"
...
rdp-info: env
	...
	echo "Training website: http://<VM_IP>:$$WEBSITE_HTTP_HOST_PORT or http://site.$$GOS_DOMAIN inside the lab"; \
